...
# ==============================================================================
    ...
    pdf.set_font("Helvetica", "B", 12); pdf.cell(0, 10, "DADOS DO CLIENTE E LOCALIDADE", ln=True)
    ...
    pdf.cell(0, 8, f"Endereco da Execucao: {endereco_disp}", ln=True); pdf.ln(8)

    pdf.set_font("Helvetica", "B", 12); pdf.cell(0, 10, "ESCOPO TÉCNICO DA PROPOSTA", ln=True)
    pdf.set_font("Helvetica", "", 11); pdf.multi_cell(190, 8.5, f"{orc_descricao if orc_descricao else 'Execucao conforme escopo acordado.'}"); pdf.ln(8)

    pdf.set_font("Helvetica", "B", 12); pdf.cell(0, 10, "DETALHAMENTO NOMINAL DOS ITENS DO PROJETO", ln=True)
    pdf.set_font("Helvetica", "B", 11); pdf.cell(0, 8, "SERVIÇOS DE MÃO DE OBRA CONTRATADOS:", ln=True)
    pdf.set_font("Helvetica", "", 11)
    ...
    pdf.ln(4); pdf.set_font("Helvetica", "B", 11); pdf.cell(0, 8, "ATIVIDADES CONCEDIDAS COMO BÔNUS (CORTESIA):", ln=True)
    pdf.set_font("Helvetica", "", 11)
    ...
    pdf.ln(8); pdf.set_font("Helvetica", "B", 12); pdf.cell(0, 10, "COMPOSIÇÃO FINANCEIRA DO PROJETO", ln=True)
    pdf.set_font("Helvetica", "", 11)
    ...
    try:
        ...
        if qr_res.status_code == 200:
            ...
            pdf.set_y(y_condicoes + 39); pdf.set_x(158); pdf.set_font("Helvetica", "B", 8.5); pdf.cell(38, 5, "Aprovar via WhatsApp", ln=True, align="C")
