...
# Inicialização de estados e tabelas fixas em memória
...
with a_calc:
    ...
    with st.form("c_v", clear_on_submit=True):
        ...
        if st.form_submit_button("💾 Salvar Veículo") and vm and mo:
            st.session_state.db_v = pd.concat([st.session_state.db_v, pd.DataFrame([{"Tipo": vt, "Marca": vm, "Modelo": mo, "Tempo de Uso (Anos)": tp, "Consumo (Km/L)": 10.0, "Valor FIPE (R$)": vl, "Seguro/Doc Anual (R$)": ip, "Manutenção Mensal (R$)": mn}])], ignore_index=True)
            st.rerun()
    ...
    if not st.session_state.db_v.empty:
        if st.checkbox("Excluir Veículo"):
            ...
            if st.button("Confirmar Exclusão V"):
                st.session_state.db_v = st.session_state.db_v.drop(int(vr.split(" - "))).reset_index(drop=True)
                st.rerun()
    ...
    with st.form("c_f", clear_on_submit=True):
        ...
        if st.form_submit_button("➕ Adicionar Gasto") and ft:
            st.session_state.db_c = pd.concat([st.session_state.db_c, pd.DataFrame([{"Tipo de Gasto": ft, "Valor Mensal (R$)": fv}])], ignore_index=True)
            st.rerun()
    ...
    if not st.session_state.db_c.empty:
        if st.checkbox("Excluir Despesa"):
            ...
            if st.button("Confirmar Exclusão D"):
                st.session_state.db_c = st.session_state.db_c.drop(int(gr.split(" - "))).reset_index(drop=True)
                st.rerun()
        df_f = st.session_state.db_c.copy()
        df_f["Valor por Hora (R$)"] = (df_f["Valor Mensal (R$)"] / ht).round(2) if ht > 0 else 0.0
    ...
    sl_h = sd / ht if ht > 0 else 0.0
    c_op_h = cm + cf_h
    h_bruto = sl_h + c_op_h
    h_fin = h_bruto / (1 - (ml / 100)) if ml < 100 else h_bruto
    st.subheader("📊 Demonstrativo do Preço por Hora")
    col1, col2 = st.columns(2)
    col1.metric("Sua Hora de Trabalho Efetivo", f"R$ {sl_h:.2f}/h")
    col2.metric("Hora de Custos Operacionais", f"R$ {c_op_h:.2f}/h")
    st.markdown(f"### 🎯 Preço Final Combinado: **R$ {h_fin:.2f}/h**")
    if st.button("🚀 Gravar Preço da Hora"):
        st.session_state["pr_h_f"] = round(h_fin, 2)
        st.success("Preço salvo para uso no orçamento!")
with a_mat:
    st.header("📦 Catálogo de Materiais")
    with st.form("f_m", clear_on_submit=True):
        c1, c2 = st.columns(2)
        md = c1.text_input("Descrição:")
        mu = c1.selectbox("Unidade:", ["Un", "m", "Barra", "Saco", "Caixa", "kg"])
        mc = c2.number_input("Custo:", min_value=0.0)
        mm = c2.slider("Margem (%):", 0, 80, 30)
        if st.form_submit_button("Salvar Material") and md:
            vf = mc / (1 - (mm / 100)) if mm < 100 else mc
            st.session_state.db_m = pd.concat([st.session_state.db_m, pd.DataFrame([{"ID": f"MAT-{len(st.session_state.db_m)+1:03d}", "Descrição": md, "Unidade": mu, "Custo (R$)": mc, "Margem (%)": mm, "Valor Unitário (R$)": round(vf, 2)}])], ignore_index=True)
            st.rerun()
    ...
    if not st.session_state.db_m.empty:
        if st.checkbox("Remover Material"):
            ...
            st.session_state.db_m = st.session_state.db_m[st.session_state.db_m["ID"] != mr.split(" - ")[0]].reset_index(drop=True)
            st.rerun()
        ...
        st.session_state.db_m = df_me

with a_serv:
    st.header("🛠️ Catálogo de Serviços")
    with st.form("f_s", clear_on_submit=True):
        c1, c2 = st.columns(2)
        sd = c1.text_input("Descrição:")
        su = c1.selectbox("Unidade Cobrança:", ["Ponto", "M²", "Diária", "Hora", "Empreitada"])
        sv = c2.number_input("Preço Sugerido:", min_value=0.0)
        if st.form_submit_button("Salvar Service") and sd:
            st.session_state.db_s = pd.concat([st.session_state.db_s, pd.DataFrame([{"ID": f"SRV-{len(st.session_state.db_s)+1:03d}", "Descrição": sd, "Unidade": su, "Valor (R$)": round(sv, 2)}])], ignore_index=True)
            st.rerun()
    ...
    if not st.session_state.db_s.empty:
        if st.checkbox("Remover Serviço"):
            ...
            st.session_state.db_s = st.session_state.db_s[st.session_state.db_s["ID"] != sr.split(" - ")[0]].reset_index(drop=True)
            st.rerun()
...
with a_orc:
    ...
    val_d = c1.number_input("Validade (Dias):", min_value=1, value=10)
    dt_e = c2.date_input("Emissão:", value=dt.date.today())
    dt_v = c3.date_input("Válido Até:", value=dt.date.today()+dt.timedelta(days=int(val_d)))
    
    st.write("---"); st.subheader("弯 Vincular Itens de Mão de Obra")
    ...
    if lista_s:
        ...
        if st.button("➕ Vincular Serviço"):
            it_s = st.session_state.db_s[st.session_state.db_s["ID"] == s_sel.split(" - ")[0]].iloc[0]
    ...
    if lista_m:
        ...
        if st.button("➕ Vincular Material"):
            it_m = st.session_state.db_m[st.session_state.db_m["ID"] == m_sel.split(" - ")[0]].iloc[0]
    ...
    if not st.session_state.df_m_sel.empty:
        st.session_state.df_m_sel = st.data_editor(st.session_state.df_m_sel, use_container_width=True, num_rows="dynamic")
        st.session_state.df_m_sel["Valor Total (R$)"] = st.session_state.df_m_sel["Quantidade"] * st.session_state.df_m_sel["Valor Unitário (R$)"]
        t_mat = st.session_state.df_m_sel["Valor Total (R$)"].sum()
    else: t_mat = 0.0

    st.write("---"); st.subheader("🎁 Vincular Política de Bônus")
    with st.form("f_b", clear_on_submit=True):
        ...
        if st.form_submit_button("➕ Vincular Bônus") and bd:
            st.session_state.df_b_sel = pd.concat([st.session_state.df_b_sel, pd.DataFrame([{"ID": f"BON-{len(st.session_state.df_b_sel)+1:03d}", "Descrição": bd, "Valor (R$)": bv}])], ignore_index=True)
            st.rerun()
    t_bon = st.session_state.df_b_sel["Valor (R$)"].sum() if not st.session_state.df_b_sel.empty else 0.0
    if t_bon > 0: st.session_state.df_b_sel = st.data_editor(st.session_state.df_b_sel, use_container_width=True, num_rows="dynamic")

    st.write("---"); st.subheader("📈 Fechamento e Alíquotas de Desconto")
    ...
    st.write(f"**Subtotal Bruto:** R$ {sub_bruto:.2f} | **Total Bônus:** R$ {t_bon:.2f} | **Desconto:** R$ {v_desc:.2f}")
    st.warning(f"💳 **TOTAL A PAGAR (ATÉ 10X CARTÃO): R$ {tot_cartao:.2f}** (Taxa de 8% inclusa)")
    ...
    def build_pdf_fenix(lf, q_url, h_val):
        ...
        t_hdr = Table([[RLImage(BytesIO(qb.getvalue()), width=45, height=45), Paragraph(tx_emp, ParagraphStyle('C', parent=s['Normal'], fontSize=8, leading=11, alignment=1)), l_bx]], colWidths=[65, 390, 100]); t_hdr.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'MIDDLE')])); sty.append(t_hdr); sty.append(Spacer(1, 10))
        ...
        t1 = Table(d_mo, colWidths=[70, 375, 110]); t1.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#1A365D')), ('TEXTCOLOR', (0,0), (-1,0), colors.whitesmoke), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#CBD5E0')), ('PADDING', (0,0), (-1,-1), 4)])); sty.append(t1); sty.append(Spacer(1, 10))
        ...
        t2 = Table(d_ma, colWidths=[65, 230, 40, 50, 85, 85]); t2.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#2B6CB0')), ('TEXTCOLOR', (0,0), (-1,0), colors.whitesmoke), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#CBD5E0')), ('PADDING', (0,0), (-1,-1), 4)])); sty.append(t2); sty.append(Spacer(1, 10))
        ...
        if t_bon > 0:
            ...
            t3 = Table(d_bo, colWidths=[70, 375, 110]); t3.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#4A5568')), ('TEXTCOLOR', (0,0), (-1,0), colors.whitesmoke), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#CBD5E0')), ('PADDING', (0,0), (-1,-1), 4)])); sty.append(t3); sty.append(Spacer(1, 10))
        ...
        t4 = Table(d_ch, colWidths=[385, 170]); t4.setStyle(TableStyle([('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#CBD5E0')), ('FONTNAME', (0,-2), (1,-1), 'Helvetica-Bold'), ('BACKGROUND', (0,-2), (0,-2), colors.HexColor('#FED7D7')), ('BACKGROUND', (0,-1), (1,-1), colors.HexColor('#C6F6D5')), ('PADDING', (0,0), (-1,-1), 4)])); sty.append(t4); doc.build(sty); bf.seek(0); return bf.getvalue()
