...
with a_orc:
    ...
    def build_pdf_fenix(lf, q_url, h_val):
        ...
        msg = f"<b>Assinado digitalmente por:</b> FÊNIX ENGENHARIA<br/><b>Data da Chancelagem:</b> {now_t} | <b>Padrão:</b> ICP-Brasil Equivalente V2<br/><b>Chave Identificadora de Autenticidade (MD5):</b> {h_val}" tgv = Table([[RLImage(BytesIO(qb.getvalue()), width=55, height=55), Paragraph(msg, ParagraphStyle('G', parent=s['Normal'], fontSize=7.5, leading=10))]], colWidths=[70, 450])tgv.setStyle(TableStyle([('BOX', (0,0), (-1,-1), 1, colors.HexColor('#A0AEC0')), ('BACKGROUND', (0,0), (-1,-1), colors.HexColor('#F7FAFC')), ('VALIGN', (0,0), (-1,-1), 'MIDDLE'), ('PADDING', (0,0), (-1,-1), 6)]))sty.append(Spacer(1, 15)); sty.append(tgv)doc.build(sty); bf.seek(0); return bf.getvalue()if st.button("🚀 Chancelar Proposta Comercial"):hv = hashlib.md5(f"{nc}{tot_vista}".encode()).hexdigest(); l_wp = f"whatsapp.com{wpp_num}&text=Aprovar%20Orcamento%20{hv}"pdf = build_pdf_fenix(logo_upload, l_wp, hv)st.download_button(label="📥 Baixar Proposta Comercial em PDF", data=pdf, file_name=f"Proposta_Fenix_{nc}.pdf", mime="application/pdf")
